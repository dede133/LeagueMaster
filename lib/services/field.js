...
export const addField = async (formData) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/field/add-field`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
...
export const getUserFields = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/field/user-fields`, {
      method: 'GET',
      credentials: 'include',
    });
...
export const getFieldDetails = async (field_id) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/field/${field_id}`);
    if (!response.ok) {
