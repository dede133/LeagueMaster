export const getFieldAvailability = async (fieldId) => {
  try {
    ...
    return data; // Retorna los datos tal como los recibes, sin mapear
  } catch (error) {
...
export const getBlockedDates = async (fieldId, startDate, endDate) => {
  try {
    const response = await fetch(
      `http://localhost:5000/api/availability/field/${fieldId}/blocked-dates?start_date=${startDate}&end_date=${endDate}`,
      {
        method: 'GET',
        credentials: 'include',
      }
